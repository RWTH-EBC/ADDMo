...
def iterative_evaluation(
    ...
    n_folds = (
        len(TestData) / horizon
    )  # get how many times the horizon fits into the data
    n_folds = int(n_folds)  # cut of incomplete horizons
    # TunedData.index = range(len(TunedData)) #give them dataframe an counter index
    (TestData_X, TestData_Y) = SVF.split_signal_and_features(
        MT_Setup_Object_PO.name_of_target, TestData
    )

    if os.path.isfile(
        os.path.join(MT_Setup_Object_PO.abs_path_to_result_folder, "ScalerTracker.save")
    ):  # if scaler was used
    ...
    for i in range(n_folds):
        ...
        measured_fold = ScaleTracker_Signal.inverse_transform(
            SVF.reshape(measured_fold)
        )
...
def predict(MT_Setup_object_PO, NameOfPredictor, _X_test):
    ...
    if os.path.isfile(
        os.path.join(
            MT_Setup_object_PO.ResultsFolderSubTest,
            "BestModels",
            "%s.save" % (NameOfPredictor),
        )
    ):  # to find out which indivmodel was used
        ...
        os.path.join(
            MT_Setup_object_PO.ResultsFolderSubTest,
            "BestModels",
            "23_%s.save" % (NameOfPredictor),
        )
    ):  # for hourly models
        indiv_predictor = indiv_model_onlypredict(
        ...
        os.path.join(
            MT_Setup_object_PO.ResultsFolderSubTest,
            "BestModels",
            "above_%s.save" % (NameOfPredictor),
        )
    ):  # for byFeature models
        byFeaturesplitter = byfeature_splitter(
            MT_Setup_object_PO.IndivThreshold, MT_Setup_object_PO.IndivFeature, _X_test
        )
        indiv_predictor = indiv_model_onlypredict(
            ...
            NameOfPredictor=NameOfPredictor,
            Recursive=MT_Setup_object_PO.OnlyPredictRecursive,
        )  # Todo: best models "byfeature" auch mit feature und threshold im namen abspeichern oder irgendwie damit das predicten unabhängig von den aktuellen werten von indivFeature usw. ist
        Predicted = indiv_predictor.main()
...
def mean_scoring(
    fold_list, errormetric
):  # processes the list of scores from "iterative_evaluation"
...
def only_predict(
    ...
    if type(Predicted) == bool:
        print(
            "There is no trained model of %s to do OnlyPredict, if needed set OnlyPredict=False and fit a model first."
            % NameOfPredictor
        )  # stop function if specific BestModel is not present
        return
    timeend = time.time()
    ComputationTime = timeend - timestart

    Y_test, Y_Predicted = SVF.apply_scaler(
        MT_Setup_object_PO, Predicted, _Y_test, Indexer
    )
    Scores = SVF.getscores(
        Y_test, Y_Predicted
    )  # Todo: Make possible to set scoring function by yourself

    Document.documentation_only_predict(
        MT_Setup_object_PO,
        RR_Model_Summary,
        NameOfPredictor,
        Y_Predicted,
    ...
    Document.visualization(
        MT_Setup_object_PO, NameOfPredictor, Y_Predicted, Y_test, Scores
    )
    ...
    if (
        ...
        MeanErrorData = Data
    fold_list = iterative_evaluation(
        MT_Setup_object_PO,
        TestData=MeanErrorData,
        Model=predict,
        horizon=horizon,
        NameOfPredictor=NameOfPredictor,
    )

    mean_score, SD_score, errorlist, errormetric = mean_scoring(
        fold_list=fold_list, errormetric=r2_score
    )
    Document.documentation_iterative_evaluation(
        MT_Setup_object_PO,
        NameOfPredictor,
        mean_score,
        SD_score,
    ...
    mean_score, SD_score, errorlist, errormetric = mean_scoring(
        fold_list=fold_list, errormetric=mean_absolute_error
    )
    Document.documentation_iterative_evaluation(
        MT_Setup_object_PO,
        NameOfPredictor,
    ...
    mean_score, SD_score, errorlist, errormetric = mean_scoring(
        fold_list=fold_list, errormetric=mean_absolute_percentage_error
    )
    Document.documentation_iterative_evaluation(
        MT_Setup_object_PO,
        NameOfPredictor,
...
def main_OnlyPredict(MT_Setup_object_PO):
    ...
    _X_train, _Y_train, _X_test, _Y_test, Indexer, Data = MT.pre_handling(
        MT_Setup_object_PO, True
    )

    OnlyPredictFolder = os.path.join(
        MT_Setup_object_PO.ResultsFolderSubTest,
        "OnlyPredict",
    ...
    for NameOfPredictor, ModelSummaryObject in zip(
        PD.AvailablePredictors, ModelSummaryObjectList
    ):
        only_predict(
            MT_Setup_object_PO,
            ModelSummaryObject,
