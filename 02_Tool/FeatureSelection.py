...
def low_variance_filter(DT_Setup_object, Data):
    ...
    filter = VarianceThreshold(
        threshold=DT_Setup_object.Threshold_LowVarianceFilter
    )  # set filter
...
def filter_univariate(DT_Setup_object, Data):
    ...
    filter = GenericUnivariateSelect(
        score_func=DT_Setup_object.Score_func,
        mode=DT_Setup_object.SearchMode,
...
def embedded__recursive_feature_selection(DT_Setup_object, Data):
    ...
    if DT_Setup_object.N_features_to_select == "automatic":
        selector = RFECV(
            estimator=DT_Setup_object.EstimatorEmbedded,
            step=1,
            cv=DT_Setup_object.CV_DT,
        )
        ...
        selector = RFE(
            estimator=DT_Setup_object.EstimatorEmbedded,
            n_features_to_select=DT_Setup_object.N_feature_to_select_RFE,
            step=1,
        )
        selector = selector.fit(X, Y)
        print("Ranks of all Features %s" % selector.ranking_)
...
def embedded__feature_selection_by_importance_threshold(DT_Setup_object, Data):
    ...
    selector = SelectFromModel(
        threshold=DT_Setup_object.Threshold_embedded, estimator=Estimator, prefit=True
    )
    Features_transformed = selector.transform(X)
    Data = SVF.merge_signal_and_features_embedded(
        DT_Setup_object.NameOfSignal,
...
def wrapper__recursive_feature_selection(DT_Setup_object, Data):
    ...
    Result_dic = DT_Setup_object.EstimatorWrapper(
        X_train, y_train, X_test, y_test, *DT_Setup_object.WrapperParams
    ...
    while True:  # loop as long as deleting features increases accuracy
        ...
        for column in X_i:  # loop through all columns
            ...
            Result_dic = DT_Setup_object.EstimatorWrapper(
                X_train_ii, y_train, X_test_ii, y_test, *DT_Setup_object.WrapperParams
            )  # score will be done over hold out 0.25 percent of data
        ...
        if Score_i > (
            Score + DT_Setup_object.MinIncrease
...
def main(DT_Setup_object, DT_RR_object):
    ...
    if (
        DT_Setup_object.AutomaticTimeSeriesOwnlagConstruct == True
    ...
    Data.to_pickle(
        os.path.join(
            DT_Setup_object.PathToPickles, "ThePickle_from_FeatureSelection" + ".pickle"
        )
    )
