...
def svr_bayesian_predictor(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV, Max_evals,
                           Recursive=False):
    ...
    def hyperopt_cv(params):
        ...
        t_end = time.time()
        print("Params per iteration: %s \ with the cross-validation score %.3f, took %.2fseconds" % (
    ...
    Best_trained_model = SVR(**BestParams).fit(Features_train,
                                               Signal_train)  # set the best hyperparameter to the SVR machine
    if not Features_test.empty:
        if Recursive == False:
            predicted = Best_trained_model.predict(Features_test)
        elif Recursive == True:
...
def rf_predictor(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid=NotImplemented,
                 CV=NotImplemented, Max_evals=NotImplemented, Recursive=False):
...
def gradientboost_gridsearch(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV,
                             Max_evals=NotImplemented, Recursive=False):
...
def gradientboost_bayesian(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV, Max_evals,
                           Recursive=False):
    ...
    Best_trained_model = GradientBoostingRegressor(**BestParams).fit(Features_train,
                                                                     Signal_train)  # set the best hyperparameter to the SVR machine
    if not Features_test.empty:
        if Recursive == False:
            predicted = Best_trained_model.predict(Features_test)
...
def lasso_grid_search_predictor(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV,
                                Max_evals=NotImplemented, Recursive=False):
...
def lasso_bayesian(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV, Max_evals,
                   Recursive=False):
    ...
    def hyperopt_cv(params):
        ...
        Estimator = linear_model.Lasso(**params,
                                       max_iter=1000000)  # give the specific parameter sample per run from fmin
        CV_score = cross_val_score(estimator=Estimator, X=Features_train, y=Signal_train, cv=CV,
                                   scoring="r2").mean()  # create a crossvalidation score which shall be optimized
        t_end = time.time()
        print("Params per iteration: %s \ with the cross-validation score %.3f, took %.2fseconds" % (
    ...
    BestParams = fmin(f, HyperparameterGrid, algo=tpe.suggest, max_evals=Max_evals,
                      trials=trials)  # do the bayesian optimization
    Best_trained_model = linear_model.Lasso(**BestParams, max_iter=1000000).fit(Features_train,
                                                                                Signal_train)  # set the best hyperparameter to the SVR machine
    if not Features_test.empty:
        if Recursive == False:
            predicted = Best_trained_model.predict(Features_test)
...
def ann_grid_search_predictor(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV,
                              Max_evals=NotImplemented, Recursive=False):
...
def ann_bayesian_predictor(Features_train, Signal_train, Features_test, Signal_test, HyperparameterGrid, CV, Max_evals,
                           Recursive=False):
