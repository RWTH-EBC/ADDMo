...
def cross_auto_cloud_correlation_plotting(DT_Setup_object, LagsToBePlotted, Data):
    ...
    for i in range(0, len(FeatureLag)):
        ...
        if NameOfFeature != SV.NameOfSignal:  # making sure this method does not produce OwnLags
            ...
            for lag in range(0, len(FeatureLag[i])):
                ...
                DataShift = Xauto.shift(periods=FeatureLag[i][lag]).fillna(
...
def difference_create(DT_Setup_object, FeaturesDifference, Data):
    ...
    for column in X:  # loop through all columns
        ...
        Difference = Difference.fillna(
            0)  # fill up first row with 0 (since the first row has no value before and hence should be 0)
        Difference = SV.post_scaler(Difference, DT_Setup_object.StandardScaling, DT_Setup_object.RobustScaling)
        Data = pd.concat([Data, Difference.rename(columns={0: DifferenceName})], axis=1,
                         join="inner")  # joining the dataframes just for the selected period
    return Data
    ...
    for i in range(0, len(Columns)):
        ...
        if NameOfFeature != SV.NameOfSignal:  # making sure this method does not produce OwnLags
            ...
            for lag in range(MinFeatureLag, (MaxFeatureLag + 1)):
                ...
                Result_dic = Estimator(X_train_i, y_train, X_test_i, y_test,
                                       *params)  # score will be done over hold out 0.25 percent of data
                ...
                if Score_2 > Score_1:
                    ...
                    Score_best = Score_2
            if Score_best > (
                    Score + MinIncrease):  # if best featurelag of respective feature is better than initial score: add it
...
def main(DT_Setup_object, DT_RR_object):
    ...
    Datas = [
        Data]  # also for not making e.g. featurelagcreate create lags of differences; Data needs to be in for the case no feature construction is done
    if DT_Setup_object.Cross_auto_cloud_correlation_plotting == True:
        cross_auto_cloud_correlation_plotting(DT_Setup_object, LagsToBePlotted=DT_Setup_object.LagsToBePlotted, Data=Data)
    if DT_Setup_object.DifferenceCreate == True:
        _Data = difference_create(DT_Setup_object, DT_Setup_object.FeaturesDifference, Data)
    ...
    if DT_Setup_object.ManFeaturelagCreate == True:
        _Data = manual_featurelag_create(DT_Setup_object.FeatureLag, Data, Data_AllSamples)
        Datas.append(_Data.drop(list(Data), axis=1))
    if DT_Setup_object.AutoFeaturelagCreate == True:
        _Data = auto_featurelag_constructor(Data, Data_AllSamples, DT_Setup_object.MinFeatureLag,
                                            DT_Setup_object.MaxFeatureLag,
                                            DT_Setup_object.EstimatorWrapper, DT_Setup_object.WrapperParams,
                                            DT_Setup_object.MinIncrease)
        Datas.append(_Data.drop(list(Data), axis=1))
    if DT_Setup_object.ManOwnlagCreate == True:
        _Data = man_ownlag_create(DT_Setup_object.OwnLag, Data, Data_AllSamples)
        Datas.append(_Data.drop(list(Data), axis=1))
