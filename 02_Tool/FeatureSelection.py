...
def man_feature_select(DT_Setup_object, FeatureSelect, Data):
    if not DT_Setup_object.ColumnOfSignal in FeatureSelect:  # keep the column of signal
...
def embedded__recursive_feature_selection(Data, Estimator, N_features_to_select, CV):
    ...
    if N_features_to_select == "automatic":
        ...
        print("Ranks of all Features %s" % selector.ranking_)
        Features_transformed = selector.transform(X)
    Data = SV.merge_signal_and_features_embedded(X_Data=X, Y_Data=Y, support=selector.get_support(indices=True),
                                                 X_Data_transformed=Features_transformed)
    return Data
...
def wrapper__recursive_feature_selection(Data, Estimator, params, MinIncrease):
    ...
    Result_dic = Estimator(X_train, y_train, X_test, y_test,
                           *params)  # score will be done over hold out 0.25 percent of data
    ...
    while True:  # loop as long as deleting features increases accuracy
        ...
        for column in X_i:  # loop through all columns
            ...
            Result_dic = Estimator(X_train_ii, y_train, X_test_ii, y_test,
                                   *params)  # score will be done over hold out 0.25 percent of data
        ...
        if Score_i > (
                Score + MinIncrease):  # is new score higher than the old one? (take care that >= would not work, since in the case that no new score was set, score_i is equal to score
...
def main(DT_Setup_object, DT_RR_object):
    ...
    if DT_Setup_object.ManFeatureSelect == True:
        Data = man_feature_select(DT_Setup_object, DT_Setup_object.FeatureSelect, Data)
    if DT_Setup_object.LowVarianceFilter == True:
        Data = low_variance_filter(Data, DT_Setup_object.Threshold_LowVarianceFilter)
    if DT_Setup_object.ICA == True:
        Data = filter_ica(Data)
    if DT_Setup_object.UnivariateFilter == True:
        Data = filter_univariate(Data, DT_Setup_object.Score_func, DT_Setup_object.SearchMode, DT_Setup_object.Param_univariate_filter)
    if DT_Setup_object.EmbeddedFeatureSelectionThreshold == True:
        Data = embedded__feature_selection_by_importance_threshold(Data=Data, Estimator=DT_Setup_object.EstimatorEmbedded,
                                                                   Threshold_embedded=DT_Setup_object.Threshold_embedded)
    if DT_Setup_object.RecursiveFeatureSelection == True:
        Data = embedded__recursive_feature_selection(Data=Data, Estimator=DT_Setup_object.EstimatorEmbedded,
                                                     N_features_to_select=DT_Setup_object.N_feature_to_select_RFE, CV=DT_Setup_object.CV_DT)
    if DT_Setup_object.WrapperRecursiveFeatureSelection == True:
        Data = wrapper__recursive_feature_selection(Data=Data, Estimator=DT_Setup_object.EstimatorWrapper, params=DT_Setup_object.WrapperParams,
                                                    MinIncrease=DT_Setup_object.MinIncrease)
    if DT_Setup_object.AutomaticTimeSeriesOwnlagConstruct == True:  # method from FeatureConstruction
        Data = automatic_timeseries_ownlag_constructor(Data=Data, Data_AllSamples=pd.read_pickle(
            os.path.join(SV.PathToPickles, "ThePickle_from_Preprocessing" + '.pickle')), MinOwnLag=DT_Setup_object.MinOwnLag,
                                                       Estimator=DT_Setup_object.EstimatorWrapper, params=DT_Setup_object.WrapperParams,
                                                       MinIncrease=DT_Setup_object.MinIncrease)
    endTime = time.time()
