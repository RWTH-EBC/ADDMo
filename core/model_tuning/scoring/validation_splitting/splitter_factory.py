...
class SplitterFactory:
    ...
    def splitter_factory(config: ModelTuningSetup) -> AbstractSplitter:
        ...
        if hasattr(custom_splitters, config.validation_score_splitting):
            custom_splitter_class = getattr(custom_splitters, config.validation_score_splitting)
        ...
        # if splitter is from scikit-learn
            ...
            scikit_learn_splitter_class = getattr(model_selection, config.validation_score_splitting)
            return scikit_learn_splitter_class(**config.validation_score_splitting_kwargs)

        else:
            # Filter out the class names of custom splitters
            custom_splitter_names = [name for name, obj in inspect.getmembers(custom_splitters)
                                     if inspect.isclass(obj) and
                                     issubclass(obj, AbstractSplitter)]
