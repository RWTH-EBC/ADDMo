...
temperature as signal. """
    ...
    def compute_FB(self, widget):
        ...
        def saveCallbackFB(OwnlagType):
            ...
            DT_Setup_Object_AFB.ColumnOfSignal = int(
    ...
    def compute_DT(self, widget):
        ...
        def saveCallbackDT():
            ...
            if DT_Setup_Object.Resample == True:
                ...
                DT_Setup_Object.WayOfResampling = convert_string_to_list(
                    self.WayOfResamplingDT.get_value(), operatordic
                )
            DT_Setup_Object.InitManFeatureSelect = (
                self.InitManFeatureSelectDT.get_value()
            )
            if DT_Setup_Object.InitManFeatureSelect == True:
                DT_Setup_Object.InitFeatures = convert_string_to_list(
            ...
            if DT_Setup_Object.Cross_auto_cloud_correlation_plotting == True:
                DT_Setup_Object.LagsToBePlotted = int(
            ...
            if DT_Setup_Object.DifferenceCreate == True:
                DT_Setup_Object.FeaturesDifference = (
                    self.FeaturesDifferenceAllDT.get_value()
                )
                if DT_Setup_Object.FeaturesDifference != True:
                    DT_Setup_Object.FeaturesDifference = convert_string_to_list(
            ...
            if DT_Setup_Object.ManOwnlagCreate == True:
                DT_Setup_Object.OwnLag = convert_string_to_list(
            ...
            if DT_Setup_Object.ManFeatureSelect == True:
                DT_Setup_Object.FeatureSelect = convert_string_to_list(
            ...
            if DT_Setup_Object.LowVarianceFilter == True:
                DT_Setup_Object.Threshold_LowVarianceFilter = float(
            ...
            if DT_Setup_Object.RecursiveFeatureSelection == True:
                ...
                if DT_Setup_Object.N_feature_to_select_RFE != "automatic":
                    DT_Setup_Object.N_feature_to_select_RFE = int(
                ...
                for key in DicCVTypes:
                    if key == self.RFE_CV_DT.get_key():
                        DT_Setup_Object.CV_DT = DicCVTypes[key](
                            (int(self.RFE_CVFolds_DT.get_value()))
                        )
            DT_Setup_Object.WrapperRecursiveFeatureSelection = (
