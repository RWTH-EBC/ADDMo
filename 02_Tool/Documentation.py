...
def documentation_DataTuning(DT_Setup_object, timestart, timeend):
    ...
    joblib.dump(
        DT_Setup_object.NameOfSignal,
        os.path.join(DT_Setup_object.ResultsFolder, "NameOfSignal.save"),
    )
    ...
    DfMethodology.at[1, "GlobalVariables"] = (
        "NameOfData = %s" % DT_Setup_object.NameOfData
    )
    DfMethodology.at[2, "GlobalVariables"] = (
        "NameOfExperiment = %s" % DT_Setup_object.NameOfExperiment
    )
    DfMethodology.at[3, "GlobalVariables"] = (
        "NameOfSignal = %s" % DT_Setup_object.NameOfSignal
    )
    DfMethodology.at[4, "GlobalVariables"] = (
        "Model used for Wrappers = %s" % DT_Setup_object.EstimatorWrapper.__name__
    )
    DfMethodology.at[5, "GlobalVariables"] = (
        "Parameter used for Wrappers = %s" % DT_Setup_object.WrapperParams
    )
    DfMethodology.at[6, "GlobalVariables"] = (
        "MinIncrease for Wrappers = %s" % DT_Setup_object.MinIncrease
    )
    ...
    DfMethodology.at[1, "Preprocessing"] = (
        "How to deal NaN´s = %s" % DT_Setup_object.NaNDealing
    )
    DfMethodology.at[2, "Preprocessing"] = (
        "Initial feature select = %s" % DT_Setup_object.InitManFeatureSelect
    )
    if DT_Setup_object.InitManFeatureSelect == True:
        DfMethodology.at[3, "Preprocessing"] = (
            "Features selected = %s" % DT_Setup_object.InitFeatures
        )
    DfMethodology.at[4, "Preprocessing"] = (
        "StandardScaler = %s" % DT_Setup_object.StandardScaling
    )
    DfMethodology.at[5, "Preprocessing"] = (
        "RobustScaler = %s" % DT_Setup_object.RobustScaling
    )
    ...
    DfMethodology.at[6, "FeatureSelection"] = (
        "Univariate Filter = %s" % DT_Setup_object.UnivariateFilter
    )
    if DT_Setup_object.UnivariateFilter == True:
        DfMethodology.at[7, "FeatureSelection"] = (
            "Score function= %s" % DT_Setup_object.Score_func
        )
        DfMethodology.at[8, "FeatureSelection"] = (
            "Search mode= %s" % DT_Setup_object.SearchMode
        )
        DfMethodology.at[9, "FeatureSelection"] = (
            "Search mode threshold parameter= %s"
            % DT_Setup_object.Param_univariate_filter
        )
    DfMethodology.at[
        10, "FeatureSelection"
    ] = "Embedded-Recursive Feature Selection = %s" % (
        DT_Setup_object.RecursiveFeatureSelection
        or DT_Setup_object.EmbeddedFeatureSelectionThreshold
    )
    if (
        DT_Setup_object.RecursiveFeatureSelection
        or DT_Setup_object.EmbeddedFeatureSelectionThreshold
        ...
        if DT_Setup_object.EmbeddedFeatureSelectionThreshold == True:
            DfMethodology.at[15, "FeatureSelection"] = (
                "Feature importance threshold = %s" % DT_Setup_object.Threshold_embedded
            )
    ...
    ExcelFile = os.path.join(
        ...
        "Settings_%s.xlsx" % (DT_Setup_object.NameOfExperiment),
    )
...
def documentation_only_predict(
    ...
    Y_Predicted.to_frame(name=MT_Setup_Object_PO.NameOfSignal).to_excel(
        SaveFileName_excel
    )
