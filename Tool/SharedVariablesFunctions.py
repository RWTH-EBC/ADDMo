...
def split_signal_and_features(target_name, Data):
...
def post_scaler(Data, StandardScaling, RobustScaling):
    ...
    try:  # works only for dataframes not Series
        ...
        if StandardScaling == True:
            ...
            Scaled_Data = mapper.fit_transform(array)  # fit it and scale the data
        # Doing "RobustScaler"
        if RobustScaling == True:
...
def apply_scaler(MT_Setup_Object, Y_test, Y_Predicted, Indexer):
    if os.path.isfile(
        os.path.join(MT_Setup_Object.abs_path_to_result_folder, "ScalerTracker.save")
    ):  # if scaler was used
...
class setup_object_initializer:
    ...
    def dts(self):
        ...
        DT_Setup_Object.abs_path_to_data = self.PathToData
        DT_Setup_Object.abs_path_to_result_folder = ResultsFolder
        DT_Setup_Object.path_to_pickles = PathToPickles
    ...
    def mts(self):
        ...
        MT_Setup_Object.abs_path_to_data = self.PathToData
        MT_Setup_Object.abs_path_to_result_folder = ResultsFolder
        MT_Setup_Object.path_to_pickles = PathToPickles
