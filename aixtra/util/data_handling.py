...
def split_simulation_data(
    ...
    """Splits system_data in training, validation, test and remaining splits; last column of dataframe will be used as y

    Parameters
    ----------
    df: DataFrame
        system_data to be process
    train_val_test_indices: list
        list of indices to be used as training, validation and test split
    val_fraction: float
        relative fraction of system_data points from dataIndices to be used as validation split
    test_fraction: float
            relative fraction of system_data points from dataIndices to be used as test split
    random_state: int
...
def move_true_invalid_from_training_2_validation(
    ...
    """Moves true invalid from training system_data to validation system_data."""
