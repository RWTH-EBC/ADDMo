...
def regressor_prediction(t_amb, rad_dir, u_hp, t_room):
    return sys_sim.bestest900_ODE_VL_COPcorr(t_amb, rad_dir, u_hp, t_room)
...
combinations = [
    (u_hp, t_amb, "u_hp", "t_amb"),
    (u_hp, t_room, "u_hp", "t_room"),
    (u_hp, rad_dir, "u_hp", "rad_dir"),
...
winter_dict = {"t_room": 294.15, "t_amb": 273.15, "rad_dir": 0, "u_hp":0.5}
...
for i, (x, y, x_label, y_label) in enumerate(combinations, 1):
    ...
    if x_label == "u_hp" and y_label == "t_amb":
        Z1 = prediction(Y, used_dict["rad_dir"], X, used_dict["t_room"])
        Z2 = regressor_prediction(Y, used_dict["rad_dir"], X, used_dict["t_room"])
    elif x_label == "u_hp" and y_label == "rad_dir":
        Z1 = prediction(used_dict["t_amb"], Y, X, used_dict["t_room"])
        Z2 = regressor_prediction(used_dict["t_amb"], Y, X, used_dict["t_room"])
    elif x_label == "u_hp" and y_label == "t_room":
        Z1 = prediction(used_dict["t_amb"], used_dict["rad_dir"], X, Y)
        Z2 = regressor_prediction(used_dict["t_amb"], used_dict["rad_dir"], X, Y)
    elif x_label == "t_amb" and y_label == "rad_dir":
