...
def parallel_plots_interactive(model_config, path_to_regressor=None):
    ...
    # Drop constant columns (no range)
    variable_cols = [col for col in xy_grid.columns if xy_grid[col].nunique() > 1]
    plot_data = xy_grid[variable_cols]

    norm_data = plot_data
    fig = px.parallel_coordinates(
        norm_data,
        color='y_pred',
        labels={col: col.replace('__', '\n') for col in norm_data.columns},
