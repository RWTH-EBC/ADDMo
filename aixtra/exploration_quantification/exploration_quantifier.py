...
from aixtra.extrapolation_detection.detector.detector_factory import DetectorFactory


def verify_bounds(x: pd.DataFrame, bounds: dict):
    """Verify that the system_data set does not contain values outside of the boundaries."""
    # check if all variables in the system_data set are present in the boundaries
    for variable in x.columns:
        if variable not in bounds:
            raise ValueError(f"Variable {variable} is not present in the boundaries.")

    # check if system_data set contains values outside of the boundaries
    for variable in x.columns:
...
class ExplorationQuantifier:
    ...
    def calculate_coverage(self):
        ...
        # Handle situations where one labels misses completely (e.g. all system_data inside)
