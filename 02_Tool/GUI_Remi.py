...
class AutomatedTraining(App):
    ...
    def main(self):
        ...
        # Auto Final ayes tab
        if True:
            FinalBayesContainer = gui.Widget(width=500, margin='0px auto',
            ...
            subContainerNameOfData, self.NameOfDataFB = text("Name of data (results)", self.info_NameOfData,
                                                             "TrialInput").do()
            ...
            subContainerOwnlags = gui.Widget(width="100%", margin='0px auto',
                                             style={'display': 'block', 'overflow': 'hidden', "text-align": "left"})
            self.lbl_Ownlags = gui.Label("Define which ownlags shall be considered", width="100%", height=20,
                                         margin="0px")
            ...
            subContainerFeatureConstruction = gui.Widget(width="100%", margin='0px auto',
                                                         style={'display': 'block', 'overflow': 'hidden',
                                                                "text-align": "left"})
            self.lbl_FeatureConstruction = gui.Label("Define which feature construction methods shall be considered",
                                                     width="100%", height=20, margin="0px")
            ...
            self.bt_UploadFile = gui.FileUploader(
                os.path.join(os.path.dirname(os.path.realpath(__file__)), "Data", "GUI_Uploads"), width="50%",
            ...
            self.FB_Execute = gui.Button("Start computations \"Auto Final Bayes\"", width="100%", height=30,
                                         margin="0px")
            ...
            FinalBayesContainer.append([subContainerFileUpload, subContainerNameOfData, subContainerNameOfSubTest,
                                        ...
                                        subContainerEndDateTraining, subContainerStartDateTesting,
                                        subContainerEndDateTesting,
                                        subContainerOwnlags, subContainerFeatureConstruction,
                                        subContainerColumnOfSignal, subContainerHyperBayesEval,
        ...
        if True:
            ...
            ContDT_NameOfData, self.NameOfDataDT = text("Name of data (results)", self.info_NameOfData,
                                                        "TrialInput").do()
            ContDT_NameOfExperiment, self.NameOfExperimentDT = text("Name of experiment (results)",
                                                                    self.info_NameOfExperiment, "TrialTunedData").do()
            ContDT_ColumnOfSignal, self.ColumnOfSignalDT = spinbox("Column of signal", self.info_ColumnOfSignal, 0, 0,
                                                                   float("inf")).do()
            ...
            ContDT_Preprocessing = gui.Widget(width='100%', layout_orientation=gui.Widget.LAYOUT_HORIZONTAL,
                                              margin='0px',
                                              style={'display': 'block', 'overflow': 'auto', "text-align": "left"})
            lbl_preprocessing = gui.Label("Preprocessing", width="90%", height=20, margin="10px",
            ...
            ContDT_Resample, self.ResampleDT = checkbox("Resample the data's resolution", self.info_resample, False,
                                                        self.exec_dialog_Resample).do()
            ContDT_InitManFeatureSelectDT, self.InitManFeatureSelectDT = checkbox("Manual feature pre-selection",
                                                                                  self.info_InitManFeatureSelect, False,
                                                                                  self.exec_dialog_InitManFeatureSelect).do()
            ...
            ContDT_PeriodSelection = gui.Widget(width='100%', layout_orientation=gui.Widget.LAYOUT_HORIZONTAL,
                                                margin='0px',
                                                style={'display': 'block', 'overflow': 'auto', "text-align": "left"})
            lbl_PeriodSelection = gui.Label("Period selection", width="90%", height=20, margin="10px",
                                            style={"text-align": "center"})
            ContDT_TimeSeriesPlot, self.TimeSeriesPlotDT = checkbox("Time series plotting", self.info_TimeSeriesPlot,
                                                                    False).do()
            ContDT_ManPeriodSelection, self.ManPeriodSelectionDT = checkbox("Manual period selection",
                                                                            self.info_ManPeriodSelect, False,
                                                                            self.exec_dialog_ManPeriodSelectionDT).do()
            ...
            lbl_FeatureConstruction = gui.Label("Feature Construction", width="90%", height=20, margin="10px",
                                                style={"text-align": "center"})
            ContDT_CrossAutoPlot, self.CrossAutoPlotDT = checkbox("Plot cross- & autocorrelation",
                                                                  self.info_CrossAutoPlot, False,
                                                                  self.exec_dialog_CrossAutoPlot).do()
            ContDT_Difference, self.DifferenceDT = checkbox("Difference", self.info_Difference, False,
                                                            self.exec_dialog_DifferenceDT).do()
            ContDT_ManOwnLag, self.ManOwnLagDT = checkbox("Manual ownlag construct", self.info_ManOwnLag, False,
                                                          self.exec_dialog_ManOwnLagDT).do()
            ContDT_AutoOwnLag, self.AutoOwnLagDT = checkbox("Auto time series ownlag", self.info_AutoOwnLag, False).do()
            ContDT_ManFeatureLag, self.ManFeatureLagDT = checkbox("Manual feature lag construct",
                                                                  self.info_ManFeatureLag, False,
                                                                  self.exec_dialog_ManFeatureLagDT).do()
            ContDT_AutoFeatureLag, self.AutoFeatureLagDT = checkbox("Auto feature lag construct",
                                                                    self.info_AutoFeatureLag, False,
                                                                    self.exec_dialog_AutoFeatureLagDT).do()
        ...
        if True:
            ...
            ContMT_NameOfData, self.NameOfDataMT = text("Name of data (as input)", self.info_NameOfData,
                                                        "TrialInput").do()
            ContMT_NameOfExperiment, self.NameOfExperimentMT = text("Name of experiment (as input)",
                                                                    self.info_NameOfExperiment, "TrialTunedData").do()
            ContMT_NameOfSubTest, self.NameOfSubTestMT = text("Name of subtest (results)", self.info_NameOfSubTest,
                                                              "TrialTunedModel").do()
            ContMT_StartTraining, self.StartTrainingMT = text("Start of training", self.info_StartDateTraining,
                                                              '2016-08-01 00:00').do()
            ContMT_EndTraining, self.EndTrainingMT = text("End of training", self.info_EndDateTraining,
                                                          '2016-08-14 23:45').do()
            ContMT_StartTesting, self.StartTestingMT = text("Start of testing", self.info_StartDateTesting,
                                                            '2016-08-15 00:00').do()
            ContMT_EndTesting, self.EndTestingMT = text("End of testing", self.info_EndDateTesting,
                                                        '2016-08-16 23:45').do()
            ContMT_HyperBayesEval, self.HyperBayesEvalMT = spinbox("Bayes eval hyperparameter",
                                                                   self.info_HyperBayesEval, 100, 1, 10000).do()
            ...
            self.CVTypeMT.select_by_key(
                next(iter(entries)))  # select the first item of the entries dictionary as initial value
            ...
            subcont2, self.MT_ANN, self.MT_GB = onerowcheckbox("ANN (BayesOpt)", False, "GradientBoost (BayesOpt)",
                                                               False)
            subcont3, self.MT_Lasso, self.MT_Modelselect = onerowcheckbox("Lasso (BayesOpt)", False, "ModelSelect",
                                                                          False)
            subcont4, self.MT_SVR_grid, self.MT_ANN_grid = onerowcheckbox("SVR (grid search)", False,
                                                                          "ANN (grid search)", False)
            subcont5, self.MT_GB_grid, self.MT_Lasso_grid = onerowcheckbox("GradientBoost (grid search)", False,
                                                                           "Lasso", False)
            ContMT_Model.append([self.lbl_ModelMT, subcont1, subcont2, subcont3, subcont4, subcont5])

            ContMT_IndivModel, self.IndivModelMT = dropdown("Type of \"individual model\"", self.info_IndivModelMT,
                                                            {"No": "No individual model",
                                                             "week_weekend": "Weekday/weekend model",
                                                             "hourly": "One model per hour (24models)",
                                                             "byFeature": "By feature threshold"}).do()
        ...
        if True:
            ...
            ContPO_NameOfData, self.NameOfDataPO = text("Name of data (as input)", self.info_NameOfData,
                                                        "TrialInput").do()
            ContPO_NameOfExperiment, self.NameOfExperimentPO = text("Name of experiment (as input)",
                                                                    self.info_NameOfExperiment,
                                                                    "TrialTunedData").do()
            ContPO_NameOfSubTest, self.NameOfSubTestPO = text("Name of subtest (as input)", self.info_NameOfSubTest,
                                                              "TrialTunedModel").do()
            ContPO_NameOfOnlyPredict, self.NameOfOnlyPredictPO = text("Name of prediction (results)",
                                                                      self.info_NameOfOnlyPredict,
                                                                      "TrialOnlyPredict").do()
            ContPO_StartTesting, self.StartTestingPO = text("Start of testing", self.info_StartDateTesting,
                                                            '2016-08-01 00:00').do()
            ContPO_EndTesting, self.EndTestingPO = text("End of testing", self.info_EndDateTesting,
                                                        '2016-08-16 23:45').do()

            ContPO_Recursive, self.RecursivePO = checkbox("Recursive prediction", self.info_Recursive, False,
                                                          False).do()
            ContPO_ValidationPeriod, self.ValidationPeriodPO = checkbox("Custom validation period",
                                                                        self.info_ValidationPeriodPO, False,
                                                                        self.exec_dialog_validationperiod_PO).do()
            ...
            PredictOnlyContainer.append([ContPO_NameOfData, ContPO_NameOfExperiment, ContPO_NameOfSubTest,
                                         ContPO_NameOfOnlyPredict, ContPO_StartTesting, ContPO_EndTesting,
                                         ContPO_Recursive,
    ...
    if True:
        def nothing(self, widget, value="optional"):  # work around function to have some widget without info function
            pass
...
increasing the error with each step.''')
        ...
        def info_ModelMT(self, widget):
            text = ("Select a model. BayesOpt and grid search mark the used type of hyperparameter optimization"
...
weekends model only for the weekends.""")
        ...
        def info_Scaler(self, widget):
            text = (
                "Define a scaler, information can be found on scikit-learn.org. Scaling is recommended in the very most cases.")
            set_text(self, text)

        def info_TimeSeriesPlot(self, widget):
            text = (
                "If true all features and signals are plotted and exported to the results folder. Can be used to determine faulty periods or to get an general idea of the data.")
            set_text(self, text)

        def info_ManPeriodSelect(self, widget):
            text = (
                "Select a period out of the overall data. In model tuning you can select train and test periods, so the period selection here is only meant to exclude periods or to reduce computation time of data tuning. ")
            set_text(self, text)

        def info_CrossAutoPlot(self, widget):
            text = (
                "Creates cross- & autocorrelation plots of all the features and signal. The autocorrelation plot can be used to determine significant ownlags, the crosscorrelation plot significant featurelags")
            set_text(self, text)
...
temperature as signal. """)
        ...
        def info_LowVarFilter(self, widget):
            text = (
                "Delete features which time series have a low variance (variance value is calculated for the scaled data).")
            set_text(self, text)
        ...
        def info_UnivariateFilterDT(self, widget):
            text = (
                "Several univariate filter methods. Information can be found in scikit-learn.org and in 'An Introduction to Variable and Feature Selection' from Isabelle Guyon which uses the term \"variable ranking\" for univariate")
            set_text(self, text)
        ...
        def info_InitFeatures(self, widget):
            text = (
                "Enter the column number of the features you want to keep, separated by a comma. (Start to count from 0 with first Column after Index, Column of signal needs to be counted, but will be kept in any case)")
        ...
        def info_WayOfResampling(self, widget):
            text = (
                "Define the way how the data shall be transformed. This needs to be defined for each column, separated by a comma. Possible operators are: mean, sum and median")
            self.InfoBoxResample.set_text(text)
        ...
        def info_UniFilterSearchStratDT(self, widget):
            text = (
                "Select a search strategy. Percentile selects the best x% of features, k-best the x best features. See scikit-learn.org")
            self.InfoBoxUnivariateFilterDT.set_text(text)
        ...
        def info_FeaturesRFEDT(self, widget):
            text = (
                "Enter an integer or \"automatic\". Integer selects the entered number of features, \"automatic\" finds optimal number by itself, only automatic supports cross-validation. Information can be found on scikit-learn.org (RFE). ")
            self.InfoBoxMultivariateEmbeddedDT.set_text(text)

        def info_RFE_CV_DT(self, widget):
            text = (
                "Select the type of cross-validation. Cross-validation information can be found on scikit-learn.org")
            self.InfoBoxMultivariateEmbeddedDT.set_text(text)
    ...
    def compute_MT(self, widget):
        ...
        def saveCallbackMT():
            ...
            Models = []
            CheckBoxDic = {self.MT_ANN: "ANN", self.MT_ANN_grid: "ANN_grid", self.MT_GB: "GB",
                           self.MT_GB_grid: "GB_grid", self.MT_Lasso: "Lasso", self.MT_Lasso_grid: "Lasso_grid",
                           self.MT_Modelselect: "ModelSelection", self.MT_RF: "RF", self.MT_SVR: "SVR",
                           self.MT_SVR_grid: "SVR_grid"}  # additional implemented models need to be entered here as well
...
that.''',
                ...
                self.dialog_periodOL.add_field_with_label("periodOL", "Number of lags for the period ownlag",
                                                          self.int_periodOL)
    ...
    def exec_dialog_featurelag_FB(self, widget, newValue):
        if newValue == True:
            try:
                ...
                self.dialog_featurelag.add_field_with_label("minFeatureLag", "Minimum feature lag",
                                                            self.int_minFeatureLag)
                self.dialog_featurelag.add_field_with_label("maxFeatureLag", "Maximum feature lag",
                                                            self.int_maxFeatureLag)
                self.dialog_featurelag.show(self)
        else:
            pass
    ...
    def exec_dialog_indivmodelbyfeature_MT(self, widget, ItemName):
        ...
        if Value == "byFeature":
            try:
                ...
                self.dialog_byfeature.add_field_with_label("IndivThreshold", "Value of threshold",
                                                           self.indivfeaturethreshold)
                self.dialog_byfeature.show(self)
        else:
            pass
    ...
    def exec_dialog_validationperiod_PO(self, widget, newValue):
        if newValue == True:
            try:
                ...
                self.dialog_validationperiod = gui.GenericDialog(title='Validation period',
                ...
                self.dialog_validationperiod.add_field_with_label("StartValidationPeriod", "Start validation period",
                                                                  self.StartValidationPeriod)
                self.dialog_validationperiod.add_field_with_label("EndValidationPeriod", "End validation period",
    ...
    def exec_dialog_Resample(self, widget, Value):
        if Value == True:  # only if the checkbox is ticked
            try:  # make sure that the dialog opens with the previously entered values and does not overwrite it with the default ones
                ...
                Cont2, self.WayOfResamplingDT = text("Way of resampling", self.info_WayOfResampling,
                                                     "mean, sum, mean, mean").do()
    ...
    def exec_dialog_InitManFeatureSelect(self, widget, Value):
        if Value == True:  # only if the checkbox is ticked
            try:  # make sure that the dialog opens with the previously entered values and does not overwrite it with the default ones
                ...
                self.dialog_InitManFeatureSelect = gui.GenericDialog(title='Initial manual feature selection',
                                                                     width='500px')
    ...
    def exec_dialog_DifferenceDT(self, widget, Value):
        if Value == True:  # only if the checkbox is ticked
            try:  # make sure that the dialog opens with the previously entered values and does not overwrite it with the default ones
                ...
                self.dialog_DifferenceDT = gui.GenericDialog(title='Difference',
    ...
    def exec_dialog_AutoFeatureLagDT(self, widget, newValue):
        if newValue == True:
            try:
                ...
                self.dialog_featurelag_DT.add_field_with_label("minFeatureLag", "Minimum feature lag",
                                                               self.MinFeatureLagDT)
                self.dialog_featurelag_DT.add_field_with_label("maxFeatureLag", "Maximum feature lag",
                                                               self.MaxFeatureLagDT)
                self.dialog_featurelag_DT.show(self)
        else:
            pass
    ...
    def exec_dialog_ManFeatureLagDT(self, widget, Value):
        if Value == True:  # only if the checkbox is ticked
            try:  # make sure that the dialog opens with the previously entered values and does not overwrite it with the default ones
                ...
                self.dialog_ManFeatureLagDT = gui.GenericDialog(title='Manual feature lag construction',
                ...
                self.dialog_ManFeatureLagDT.add_field_with_label("ManFeatureLagDT", "List of feature lags",
    ...
    def exec_dialog_UnivariateFilterDT(self, widget, Value):
        if Value == True:  # only if the checkbox is ticked
            try:  # make sure that the dialog opens with the previously entered values and does not overwrite it with the default ones
                ...
                Cont1, self.UniFilterScorFuncDT = dropdown("Type of score function", self.info_UniFilterScoreFunc,
                                                           {mutual_info_regression: "mutual information regression",
                                                            f_regression: "f regression"}).do()
                Cont2, self.UniFilterSearchStratDT = dropdown("Search strategy", self.info_UniFilterSearchStratDT,
                                                              {"percentile": "percentile", "k_best": "k-best"}).do()
                Cont3, self.UniFilterParamDT = text("Parameter for search strategy", self.info_UniFilterParamDT,
                                                    "").do()
...
start(AutomatedTraining, address='0.0.0.0', port=8081, multiple_instance=True, enable_file_cache=True,
      update_interval=0.9, start_browser=True)
