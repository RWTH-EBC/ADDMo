...
from aixtra.extrapolation_detection.detector.detectors import D_KNN, D_OCSVM, D_ParzenWindow, D_GP, D_IF, \
    D_ABOD, D_LOF, D_MCD, D_GMM, \
    D_HBOS, D_ECOD, D_DSVDD, D_RNN, D_PCA, D_FB_KNN
from aixtra.extrapolation_detection.detector.abstract_detector import AbstractDetector
from aixtra.extrapolation_detection.detector.scoring import score_samples
...
class AbstractHyper(ABC):
    ...
    def get_clf(self, x_train: ndarray, x_val: ndarray, groundtruth_val: ndarray) \
        ...
        x_val: ndarray
            MxD matrix of validation system_data, M: number of system_data points, D: number of dimensions
        groundtruth_val: ndarray
             Mx1 matrix, M: number of validation system_data points; Classification: 0: Normal system_data, 1: Outlier;
        groundtruth_train: ndarray
            Optional Nx1 matrix, N: number of training system_data points; Classification: 0: Normal system_data, 1: Outlier;
            If not None, training system_data will be added to validation system_data for scoring
...
class Hyper_Threshold:
    ...
    def opt(self, groundtruth: ndarray, nscores: ndarray) -> float:
        ...
        ----------
        groundtruth: ndarray
             Mx1 matrix, M: number of system_data points; Classification: 0: Normal system_data, 1: Outlier;
        nscores: ndarray
            Mx1 matrix of novelty scores, M: number of system_data points
