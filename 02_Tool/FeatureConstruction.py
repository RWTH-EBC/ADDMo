...
def manual_featurelag_create(DT_Setup_object, Data, Data_AllSamples):
    ...
    for i in range(0, len(DT_Setup_object.FeatureLag)):
        ...
        if (
            NameOfFeature != DT_Setup_object.NameOfSignal
            ...
            for lag in range(0, len(DT_Setup_object.FeatureLag[i])):
                FeatureLagName = (
                    NameOfFeature + "_lag" + str(DT_Setup_object.FeatureLag[i][lag])
                )  # create a column name per lag
                DataShift = Xauto.shift(
                    periods=DT_Setup_object.FeatureLag[i][lag]
                ).fillna(
                    method="bfill"
...
def man_ownlag_create(DT_Setup_object, Data, Data_AllSamples):
    ...
    for lag in range(0, len(DT_Setup_object.OwnLag)):
        OwnLagName = (
            DT_Setup_object.NameOfSignal + "_lag_" + str(DT_Setup_object.OwnLag[lag])
        )  # create a column name per lag
        DataShift = Xauto.shift(periods=DT_Setup_object.OwnLag[lag]).fillna(
            method="bfill"
...
def difference_create(DT_Setup_object, Data):
    if (
        DT_Setup_object.FeaturesDifference == True
        ...
        (X, Y) = SVF.split_signal_and_features(DT_Setup_object.NameOfSignal, Data)
        X = X[
            X.columns[DT_Setup_object.FeaturesDifference]
    ...
    for column in X:  # loop through all columns
        ...
        Difference = SVF.post_scaler(
            Difference, DT_Setup_object.StandardScaling, DT_Setup_object.RobustScaling
        )
        Data = pd.concat(
...
def automatic_timeseries_ownlag_constructor(DT_Setup_object, Data, Data_AllSamples):
    ...
    while True:  # loop as long as a new ownlag increases the accuracy
        ...
        OwnLagName = (
            DT_Setup_object.NameOfSignal + "_lag_" + str(i)
        )  # create a column name per lag
...
def auto_featurelag_constructor(DT_Setup_object, Data, Data_AllSamples):
    ...
    for i in range(0, len(Columns)):
        ...
        if (
            ...
            for lag in range(
                DT_Setup_object.MinFeatureLag, (DT_Setup_object.MaxFeatureLag + 1)
            ):
                Score_1 = Score_best
                FeatureLagName = (
                ...
                (X_i, Y) = SVF.split_signal_and_features(
                    DT_Setup_object.NameOfSignal, Data_i
                )
                X_train_i, X_test_i, y_train, y_test = train_test_split(
                    X_i, Y, test_size=0.25
                )
                Result_dic = DT_Setup_object.EstimatorWrapper(
                    X_train_i, y_train, X_test_i, y_test, *DT_Setup_object.WrapperParams
                )  # score will be done over hold out 0.25 percent of data
                Score_2 = Result_dic[
                    "score"
