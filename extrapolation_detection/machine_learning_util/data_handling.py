...
def split_data(df: DataFrame, dataIndices: list, validationSplit: float, testSplit: float, random_state: int = 0,
    ...
    -------
    dict
        {'TrainingData': TrainingData, 'x_data', 'y_data', 'header'
    """

    # select list of indices used as training, validation and test split
    data = df.iloc[dataIndices]
    ...
    if shuffle:
        ...
        data_shuffled = data
    training_split = 1 - validationSplit - testSplit

    # training data
    xy_training = data_shuffled.iloc[0:round(len(data_shuffled)*training_split)]
    # validation data
    xy_validation = data_shuffled.iloc[round(len(data_shuffled)*training_split):
                                       round(len(data_shuffled)*(training_split + validationSplit))]
    # test data
    xy_test = data_shuffled.iloc[round(len(data_shuffled)*(training_split + validationSplit)):]
    ...
    data = df.drop(dataIndices)
    x_data = data.iloc[:, :-1].to_numpy()
    y_data = data.iloc[:, -1].to_numpy().reshape((-1, 1))

    return {'TrainingData': TrainingData(x_train, y_train, x_val, y_val, x_test, y_test),
            'x_data': x_data, 'y_data': y_data, 'header': np.array(df.columns)}
