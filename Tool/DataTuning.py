...
def main(DT_Setup_object: DataTuningSetup):
    ...
    if DT_Setup_object.abs_path_to_data == "Empty":
    ...
    SVF.delete_and_create_folder(DT_Setup_object.abs_path_to_result_folder)
    os.makedirs(DT_Setup_object.path_to_pickles)
    ...
    NameOfSignal = list(__Data)[DT_Setup_object.name_of_target]
    DT_Setup_object.name_of_target = NameOfSignal  # save the variable in DTS object
...
if __name__ == "__main__":
    DT_Setup_Object = DataTuningSetup()
    DT_Setup_Object = SVF.setup_object_initializer(DT_Setup_Object).dts()
